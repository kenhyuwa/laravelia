...
<html lang="{{ app()->getLocale() }}">
    ...
    @if (is_route(Route::currentRouteName()))
        <body class="hold-transition login-page with-bg modern-{{ App\Models\User::first()->my_color ?? 'purple' }}-skin">
            @yield('content')
            <script src="{{ asset(__v().'/js/app.js') }}"></script>
            @yield('js')
        </body>
    @else
        <body class="hold-transition modern-{{ auth()->user()->my_color ?? 'purple' }}-skin fixed sidebar-mini">
