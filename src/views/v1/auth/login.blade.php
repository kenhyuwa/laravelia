    ...
    <div class="login-box-body">
        <form id="login" action="{{ route('login') }}" method="post">
            ...
            <div class="form-group has-feedback{{ $errors->has('password') ? ' has-error' : '' }}">
                ...
                <input id="password" type="password" class="form-control" name="password" value="{{ App::environment('staging') ? 'Larav3lP0s' : '' }}{{ App::environment('local') ? 'secret' : '' }}">
        ...
        <a href="{{ route('password.request') }}" class="text-center is-block mt-10">I forgot my password</a><br>
        ...
        axios.post(
            ...
            if(e.response.status === 422){
                if(typeof e.response.data.errors.email !== 'undefined'){
                    failedResponse(e.response.data.errors.email[0]);
                }
            }
        });
    });
